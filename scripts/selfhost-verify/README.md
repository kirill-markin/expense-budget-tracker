...
| 8 | `GET /v1/` answers the discovery envelope with the configured `api.` and `mcp.` URLs both through the edge and straight at the published port, where the `Host` is `127.0.0.1:8083` — which is the request an unset `PUBLIC_API_BASE_URL`/`PUBLIC_AUTH_BASE_URL` pair answers with a bare 500. Both envelopes must also describe the onboarding this mode serves: no `send_code` action, no `auth.bootstrapUrl`, and an `appBaseUrl` field plus instructions naming key creation in the browser app at `https://app.<domain>` instead. |
