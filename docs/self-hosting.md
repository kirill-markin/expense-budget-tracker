...
The four `AUTH_PROXY_*` variables are set on `web` and `auth`, the two services that resolve a person's identity. `AUTH_MODE` is set on `web`, `auth` and `api`, but only `web` and `auth` resolve identity from it: `api` reads it solely to describe the right onboarding in its discovery response, alongside `PUBLIC_APP_BASE_URL`, the app origin that onboarding points at. `mcp` and `worker` read neither variable. Callers of `api`, `mcp` and `worker` authenticate with an `ApiKey` or an OAuth Bearer token, not with the edge token.
...
- **No terminal onboarding for the `/v1` machine API.** The only route that issues an agent API key from a terminal is `POST /api/agent/verify-code` on the auth service, and `proxy_jwt` does not register it. Create the key in the browser instead: **Settings → Agent and Program Access → Create an API key** mints one for the identity the edge already authenticated, shows it once, and stores only its hash. `/v1` then accepts it as `Authorization: ApiKey <key>`. One user may hold 25 un-revoked keys at once, which bounds what a stolen browser session can mint; revoked keys do not count, so rotation is never blocked. `GET /v1/` describes exactly that: because `api` is given `AUTH_MODE=proxy_jwt`, its discovery response carries no `send_code` action and no `auth.bootstrapUrl`, and its instructions name the Settings path above at `PUBLIC_APP_BASE_URL`, which the response also carries as `appBaseUrl`.
