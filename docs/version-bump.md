...
Do not replace those references with a version literal. Do not bulk-replace version literals in the dossier. Its only unmanaged product-version literals are fixed historical evidence: the historical Item 04 `server.json` summary attached to commit `8f0b330098fb8829f9f340a27501d73eb4b1860b`, the named `tools-list-v1.2.0-promotion-candidate-v1` and `tools-list-v1.6.0-promotion-candidate-v1` descriptor snapshots wherever referenced, the completed base-alignment checklist item, and every statement about an already-published MCP Registry record, including its version, publication timestamp, and credential provenance. Keep those literals unchanged, treat the managed fields above as the authoritative set of dossier values that change, and add new evidence for a later version when needed.
