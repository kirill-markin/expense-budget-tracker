...
| Version under evaluation | 1.2.0 |
...
| R07 | Authorization request, owner-controlled valid `GET`, then consent `POST` | `https://auth.expense-budget-tracker.com/oauth/authorize` | Without a session, same-origin `302` to login; after login, `200`, `text/html` consent; approval returns same-origin `302` to the registered callback | Pending controlled-client and Developer Mode evidence; record the complete sanitized chain |
...
| Runtime version | 1.2.0 unless a later aligned version is promoted |
...
| Registry manifest identity/version | `com.expense-budget-tracker/expense-budget-tracker` / `1.2.0` |
