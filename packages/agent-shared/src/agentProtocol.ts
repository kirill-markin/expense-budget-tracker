...
A category is not a stored object but a name carried by rows: it exists while a row in ledger_entries, budget_lines, or budget_adjustments names it, and it stops being shown once no row names it. Renaming a category therefore means writing the new name on every row that carries the old one across those three tables, and on any copy of it in workspace_settings.filtered_categories.
