...
export const buildAgentDiscoveryInstructions = (apiBaseUrl: string): string =>
  `Ask the user for their email address first, then call send_code. The same email OTP flow handles both signup and login. After login, save the returned key outside chat memory, preferably in a local .env file as ${AGENT_API_KEY_ENV_VAR_NAME}='<PASTE_KEY_HERE>', then call ${apiBaseUrl}/me, ${apiBaseUrl}/workspaces, and ${apiBaseUrl}/workspaces/{workspaceId}/select before SQL. Use ${apiBaseUrl}/schema to inspect allowed relations, columns, and any agent hints about constraints or write semantics. Restricted SQL does not support ON CONFLICT. Before any long mutating INSERT or UPDATE, first try the same SQL shape on a tiny representative probe: 1-3 literal rows for INSERT or 1 targeted row for UPDATE. If the probe fails, fix the SQL and retry the small version. After the probe succeeds, continue with the remaining approved data in sequential batches of at most 100 records per tool call. The user's explicit approval covers the full approved dataset across those sequential tool calls; only ask again if the requested change itself changes. Example: curl -H '${API_KEY_AUTHORIZATION_SCHEME.replace("<key>", `$${AGENT_API_KEY_ENV_VAR_NAME}`)}' ${apiBaseUrl}/me.`;
