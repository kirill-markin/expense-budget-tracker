/**
 * Postgres-backed pin for the `/v1` and MCP identity mirror.
 *
 * The unit tests read the statement text; only a real database proves what
 * the `app` role actually leaves in `public.users` after an ApiKey or MCP
 * request. Whatever account state the request carries, a disabled row stays
 * disabled, while `email_verified` still follows the authenticated identity
 * so the MCP access-token gate cannot strand a subject permanently.
 */
...
import { upsertUserIdentity, type UserIdentity } from "./db.js";

const migrationDatabaseUrl = process.env.MIGRATION_DATABASE_URL ?? "";
const appDatabaseUrl = process.env.APP_DATABASE_URL ?? "";
const databaseUrlsMissing = migrationDatabaseUrl === "" || appDatabaseUrl === "";
const MISSING_DATABASE_URLS_MESSAGE =
  "MIGRATION_DATABASE_URL and APP_DATABASE_URL are required for the Postgres-backed SQL API identity mirror test";
...
test(
  "an ApiKey or MCP request provisions a first-seen user and never raises a disabled account",
  ...
  async (): Promise<void> => {
    ...
    const userId = `sql-api-mirror-user-${randomUUID().replaceAll("-", "")}`;
    ...
    try {
      ...
      // email_verified is cleared alongside it to stand in for a row stored
      // before the address was marked verified: the statement must still be
      // able to repair it, because the OTP and consent surfaces that verified
      // the address share this column. The ApiKey caller of this surface
      // passes the stored value back instead (machineApi/request.test.ts).
      ...
      assert.deepEqual(
        ...
        "a request carrying an enabled identity must not re-enable the stored account, but must repair email_verified",
      );
