...
export const handleSqlRouteWithWorkspaceResolver = async (
  ...
  handleValidatedSqlRouteWithWorkspaceResolver(
    ...
    runSql,
    {
      ...
      success: `Workspace context is required for SQL. Use X-Workspace-Id to override the saved workspace for this API key. Prefer SELECT first, semicolon-separated statements are allowed, only supported relations may be queried, only allowlisted pure aggregate, date, text, cast, and window functions may be called and a rejected call lists the allowed names, returned rows are capped at ${MAX_SQL_ROWS} per statement and request with returnedRowCount, totalRowCount, and truncated metadata, and mutations are limited to ${MAX_SQL_MUTATION_ROWS} affected rows per request. A result is also capped at limits.maxResultChars (${MAX_SQL_RESULT_CHARS}) characters across the whole script; an oversized read drops rows, then referencedRelations, then cuts the echoed sql, failing with sql_result_too_large only when even that is over budget, a committed write always succeeds, and both explain the shrink in resultSizeInstructions, so send fewer statements per request when the script is long.`,
    },
  );
