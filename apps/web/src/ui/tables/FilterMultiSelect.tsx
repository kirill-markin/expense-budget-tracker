  ...
  return (
    <label className={styles.filterLabel}>
      ...
      <button
        ref={triggerRef}
        type="button"
        className={cn(styles.filterInput, styles.filterTrigger, open ? styles.filterTriggerOpen : "")}
        onClick={handleToggleOpen}
        data-testid={testId}
      >
        <span className={styles.filterTriggerText}>{summary}</span>
      </button>
  ...
  return (
    ...
    >
      <input
        ref={searchRef}
        type="text"
        className={styles.filterPopoverSearch}
        placeholder={t("txn.filterSearchPlaceholder")}
        value={search}
        onChange={(event) => setSearch(event.target.value)}
        data-testid={`${testId}-search`}
      />
      <div className={styles.filterPopoverOptions}>
        {filteredOptions.map((option) => (
          <label key={option.value} className={styles.filterPopoverOption}>
            <input
              type="checkbox"
              checked={selectedSet.has(option.value)}
              onChange={() => onToggleValue(option.value)}
            />
            <span className={styles.filterPopoverOptionLabel}>{option.label}</span>
          </label>
        ))}
        {filteredOptions.length === 0 && (
          <div className={styles.filterPopoverEmpty}>{t("txn.filterNoMatches")}</div>
        )}
      </div>
    </div>
  );
